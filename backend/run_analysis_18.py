...
def run_analysis_for_report_18():
    db = SessionLocal()
    try:
        report = db.query(Report).filter(Report.id == 18).first()
        if not report:
            print("Report 18 not found!")
            return

        # Fetch seeded biomarkers
        db_biomarkers = db.query(Biomarker).filter(Biomarker.report_id == 18).all()
        biomarkers_data = []
        ...
        print(f"Loaded {len(biomarkers_data)} biomarkers for report 18.")

        # Clean existing analysis and recommendations
        db.query(Recommendation).filter(Recommendation.analysis_id.in_(
            db.query(Analysis.id).filter(Analysis.report_id == 18)
        )).delete(synchronize_session=False)
        db.query(Analysis).filter(Analysis.report_id == 18).delete(synchronize_session=False)
        db.commit()
